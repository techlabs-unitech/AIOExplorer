...
const NETWORKS = [
  { id:"eth",       name:"Ethereum",  symbol:"ETH",   color:"#627EEA", icon:"⬡", chainId:"0x1",    ankr:"eth",       explorer:"https://etherscan.io",            rpc:"https://ethereum.publicnode.com" },
  { id:"bsc",       name:"BNB Chain", symbol:"BNB",   color:"#F3BA2F", icon:"◈", chainId:"0x38",   ankr:"bsc",       explorer:"https://bscscan.com",             rpc:"https://bsc.publicnode.com" },
  { id:"polygon",   name:"Polygon",   symbol:"MATIC", color:"#8247E5", icon:"⬟", chainId:"0x89",   ankr:"polygon",   explorer:"https://polygonscan.com",         rpc:"https://polygon.publicnode.com" },
  { id:"arbitrum",  name:"Arbitrum",  symbol:"ETH",   color:"#28A0F0", icon:"◆", chainId:"0xa4b1", ankr:"arbitrum",  explorer:"https://arbiscan.io",             rpc:"https://arbitrum-one.publicnode.com" },
  { id:"avalanche", name:"Avalanche", symbol:"AVAX",  color:"#E84142", icon:"▲", chainId:"0xa86a", ankr:"avalanche", explorer:"https://snowtrace.io",            rpc:"https://avalanche-c-chain.publicnode.com" },
  { id:"base",      name:"Base",      symbol:"ETH",   color:"#0052FF", icon:"⬤", chainId:"0x2105", ankr:"base",      explorer:"https://basescan.org",            rpc:"https://base.publicnode.com" },
  { id:"optimism",  name:"Optimism",  symbol:"OP",    color:"#FF0420", icon:"◉", chainId:"0xa",    ankr:"optimism",  explorer:"https://optimistic.etherscan.io", rpc:"https://optimism.publicnode.com" },
  { id:"fantom",    name:"Fantom",    symbol:"FTM",   color:"#1969FF", icon:"◇", chainId:"0xfa",   ankr:"fantom",    explorer:"https://ftmscan.com",             rpc:"https://fantom.publicnode.com" },
];
const CHAIN_MAP = {};
...
async function fetchAll(address, network, onStep) {
  ...
  try {
    const bal = await ankrPost("ankr_getAccountBalance",{blockchain:network.ankr,walletAddress:address,onlyWhitelisted:false,pageSize:50});
    if (bal?.assets?.length) {
      out.tokens = bal.assets.map(a=>({name:a.tokenName||a.tokenSymbol||"Unknown",symbol:a.tokenSymbol||"?",balance:fmtBal(a.balance),rawBalance:parseFloat(a.balance)||0,usd:parseFloat(a.balanceUsd)||0,valueUSD:fmtUSD(a.balanceUsd),price:a.tokenPrice?fmtUSD(a.tokenPrice):"—",priceChange:a.tokenPrice24hPercentChange?parseFloat(a.tokenPrice24hPercentChange).toFixed(2):null,thumbnail:a.thumbnail||null,tokenType:a.tokenType||"ERC20",contractAddress:a.contractAddress||null,decimals:a.tokenDecimals||18})).sort((a,b)=>b.usd-a.usd);
      out.totalUSD=out.tokens.reduce((s,t)=>s+t.usd,0); out.source="Ankr";
    }
  ...
  if(!out.tokens.length){
    ...
    try{
      ...
      for(const tk of(KNOWN_TOKENS[network.id]||[])){
        try{const c=new ethers.Contract(tk.address,ERC20_ABI,provider);const[r,d]=await Promise.all([c.balanceOf(address),c.decimals()]);const b=parseFloat(ethers.formatUnits(r,d));if(b>0)out.tokens.push({name:tk.name,symbol:tk.symbol,balance:fmtBal(b,4),rawBalance:b,usd:0,valueUSD:"—",price:"—",priceChange:null,thumbnail:null,tokenType:"ERC20",contractAddress:tk.address,decimals:d});}catch{}
      }
  ...
  try{
    const td=await ankrPost("ankr_getTransactionsByAddress",{blockchain:network.ankr,address,pageSize:25,descOrder:true,includeLogs:false});
    if(td?.transactions?.length){out.txs=td.transactions.map(t=>({hash:t.hash,from:t.from,to:t.to,value:fmtBal(parseInt(t.value||"0x0",16)/1e18,8),symbol:network.symbol,type:t.from?.toLowerCase()===address.toLowerCase()?"Send":"Receive",time:t.timestamp?timeAgo(t.timestamp):"—",status:t.status==="1"||t.status===1?"confirmed":t.status==="0"||t.status===0?"failed":"pending"}));}
  }catch(e){console.warn("Ankr txs:",e.message);}
  onStep("Fetching NFTs…",78);
  try{
    const nd=await ankrPost("ankr_getNFTsByOwner",{blockchain:network.ankr,walletAddress:address,pageSize:24});
    if(nd?.assets?.length){out.nfts=nd.assets.map(n=>({name:n.name||`#${n.tokenId}`,collection:n.collectionName||"Unknown Collection",tokenId:n.tokenId||"",image:n.imageUrl||null,contractAddress:n.contractAddress||""}));}
...
export default function App() {
  ...
  const C = {
    bg:"#060810", card:"#0a0e1d", border:"#131d34", text:"#c8d4f0",
    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'IBM Plex Mono',monospace",display:"flex",flexDirection:"column",overflowX:"hidden"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}body{background:#060810}
        ::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:#08091a}::-webkit-scrollbar-thumb{background:#1a2848;border-radius:2px}
        ...
        .card{background:#0a0e1d;border:1px solid #131d34;border-radius:10px}
        .tab{background:transparent;border:none;color:#253550;cursor:pointer;font-family:'IBM Plex Mono',monospace;font-size:10px;font-weight:600;letter-spacing:2px;padding:11px 16px;border-bottom:2px solid transparent;transition:all .25s;text-transform:uppercase}
        .tab.on{color:#7a9aff;border-bottom-color:#3a6aff}.tab:hover:not(.on){color:#405070}
        .tr{display:flex;align-items:center;justify-content:space-between;padding:13px 18px;border-bottom:1px solid #0c1324;transition:background .15s}
        .tr:last-child{border-bottom:none}.tr:hover{background:rgba(42,74,255,.03)}.tr.click{cursor:pointer}
        .net-o{display:flex;align-items:center;gap:10px;padding:9px 13px;cursor:pointer;border-radius:6px;transition:all .12s}.net-o:hover{background:#0d1525}
        .mb{position:fixed;inset:0;background:rgba(3,5,12,.92);backdrop-filter:blur(14px);z-index:400;display:flex;align-items:center;justify-content:center;padding:20px}
        .mo{background:#08101e;border:1px solid #172038;border-radius:14px;padding:28px;width:100%;max-width:460px;box-shadow:0 48px 100px rgba(0,0,0,.95)}
        .inp{width:100%;background:#05080f;border:1px solid #101c30;border-radius:6px;color:#c8d4f0;font-family:'IBM Plex Mono',monospace;font-size:13px;padding:12px 14px;outline:none;transition:border-color .2s}
        .inp:focus{border-color:rgba(58,90,255,.5)}.inp::placeholder{color:#18243a}.inp.err{border-color:rgba(255,74,106,.35)}
        ...
        .fl{color:#1e2e44;font-size:12px;text-decoration:none;transition:color .2s;cursor:pointer;line-height:2.1;display:block}
        ...
        .col-h{display:flex;justify-content:space-between;font-size:8px;letter-spacing:2.5px;color:#162038;text-transform:uppercase;padding:9px 18px;border-bottom:1px solid #0c1324}
        .tok-img{width:34px;height:34px;border-radius:50%;object-fit:cover;border:1px solid #18243a;background:#07090e;flex-shrink:0}
        .tok-ph{width:34px;height:34px;border-radius:50%;border:1px solid #18243a;background:#07090e;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;flex-shrink:0}
        .empty{text-align:center;padding:54px 20px;color:#162030;font-size:13px;line-height:2}
        .stat-b{background:#070b16;border:1px solid #101826;border-radius:8px;padding:14px 22px;text-align:center;min-width:95px}
        .chip{display:inline-block;padding:2px 7px;border-radius:3px;font-size:8px;font-weight:600;background:#091020;border:1px solid #142036;color:#2a4068;text-transform:uppercase;letter-spacing:.5px}
        .fline{height:1px;background:linear-gradient(90deg,transparent,#1a2a4433,#3a5aff1e,#1a2a4433,transparent)}
        .net-pill{display:flex;align-items:center;gap:8px;background:#090d1c;border:1px solid #14203a;border-radius:6px;padding:7px 12px;cursor:pointer;transition:all .2s;font-family:'IBM Plex Mono',monospace;font-size:12px;color:#c8d4f0}
        .net-pill:hover{border-color:rgba(42,58,102,.6);background:#0c1226}
        .gas-pill{display:flex;align-items:center;gap:5px;background:#070b16;border:1px solid #101826;border-radius:4px;padding:4px 10px;font-size:10px;color:#253550;font-family:'IBM Plex Mono',monospace}
        .clk-pill{display:flex;align-items:center;gap:5px;background:#070b16;border:1px solid #101826;border-radius:4px;padding:4px 10px}
        .nav-link{font-family:'IBM Plex Mono',monospace;font-size:10px;font-weight:500;letter-spacing:1.5px;color:#253550;cursor:pointer;text-transform:uppercase;transition:color .2s;padding:4px 0;border-bottom:1px solid transparent;text-decoration:none}
      ...
      <header style={{position:"sticky",top:0,zIndex:100,flexShrink:0}}>
        ...
        {/* ── Ticker bar ── */}
        <div style={{background:"#030508",borderBottom:"1px solid #090f1e",padding:"5px 0"}}>
          <div className="ticker-wrap">
            <div className="ticker-track">
              {[...TICKER_ITEMS,...TICKER_ITEMS].map((tk,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:7,padding:"0 24px",flexShrink:0,borderRight:"1px solid #0a1220"}}>
                  <span style={{fontSize:9,fontWeight:700,color:"#405078",letterSpacing:1.5}}>{tk.s}</span>
                  <span style={{fontSize:10,color:"#8899b8",fontWeight:500}}>{tk.p}</span>
                  <span style={{fontSize:9,color:tk.u?"#00c87a":"#ff5a5a",fontWeight:700}}>{tk.c}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* ── Main nav ── */}
        <div style={{background:"rgba(6,8,16,.96)",backdropFilter:"blur(24px)",borderBottom:"1px solid #0c1424",position:"relative"}}>
          <div className="hglow"/>
          <div style={{maxWidth:1200,margin:"0 auto",padding:"0 24px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,flexShrink:0}}>
              ...
              <div>
                <div style={{fontFamily:"'Syne',sans-serif",fontSize:19,fontWeight:900,letterSpacing:3,color:"#fff",lineHeight:1}}>CHAIN<span style={{color:net.color}}>SCAN</span></div>
                <div style={{fontSize:8,color:"#182038",letterSpacing:3.5,marginTop:2,textTransform:"uppercase"}}>Multi-Chain Explorer</div>
              </div>
            </div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
              ...
              <div className="clk-pill"><span style={{fontSize:8,color:"#182438",letterSpacing:1.5}}>UTC</span><span style={{fontSize:10,color:"#2e4860",fontWeight:600,letterSpacing:.5}}>{utcTime}</span></div>
        ...
        {/* ── Chain status bar ── */}
        <div style={{background:"#030508",borderBottom:"1px solid #09101e",padding:"5px 0"}}>
          <div style={{maxWidth:1200,margin:"0 auto",padding:"0 24px",display:"flex",alignItems:"center",gap:20,overflowX:"auto"}}>
            <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
              ...
              <span style={{fontSize:8,color:"#1a3828",letterSpacing:2,textTransform:"uppercase",whiteSpace:"nowrap"}}>All Systems Operational</span>
            </div>
            <div style={{width:1,height:10,background:"#101e30",flexShrink:0}}/>
            ...
            <div style={{marginLeft:"auto",flexShrink:0}}>
              <span style={{fontSize:8,color:"#101e30",letterSpacing:1,whiteSpace:"nowrap"}}>Powered by Ankr · PublicNode</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main style={{flex:1,maxWidth:960,width:"100%",margin:"0 auto",padding:"50px 24px 80px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:42}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"#080d1c",border:`1px solid ${net.color}2e`,borderRadius:20,padding:"5px 16px",marginBottom:18}}>
          ...
          <h1 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(32px,5.5vw,54px)",fontWeight:900,lineHeight:1.06,marginBottom:14,letterSpacing:-1.5}}>
            <span style={{color:"#fff"}}>Multi-Chain </span><span style={{color:net.color,textShadow:`0 0 80px ${net.color}33`}}>Wallet</span><br/><span style={{color:"#fff"}}>Scanner</span>
          </h1>
          <p style={{color:"#253552",fontSize:13,maxWidth:480,margin:"0 auto",lineHeight:1.85}}>Real-time token balances · BEP-20 / ERC-20 holdings<br/>NFT collections · Transaction history</p>
        </div>

        {walletErr&&<div style={{background:"#100508",border:"1px solid #30100e",borderRadius:8,padding:"11px 16px",marginBottom:18,color:"#ff7a8a",fontSize:12}}>⚠ {walletErr}</div>}

        {/* Search */}
        <div className="card" style={{padding:24,marginBottom:24,border:`1px solid ${scanning?net.color+"55":"#131d34"}`,transition:"border-color .3s",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{fontSize:8,letterSpacing:3.5,color:"#182038",fontFamily:"'Syne',sans-serif",marginBottom:12,textTransform:"uppercase"}}>Wallet Address</div>
          ...
          {addrErr&&(
            <div style={{marginTop:10,display:"flex",alignItems:"flex-start",gap:9,background:"#0e0508",border:"1px solid #30100e",borderRadius:7,padding:"10px 14px"}}>
              <span style={{color:"#ff5a5a",fontSize:17,lineHeight:1,flexShrink:0}}>⚠</span>
              <div>
                <div style={{color:"#ff7a7a",fontSize:12,fontWeight:600,marginBottom:3}}>Invalid Address</div>
                <div style={{color:"#7a3838",fontSize:11,lineHeight:1.6}}>{addrErr}</div>
                <div style={{color:"#3e1e1e",fontSize:10,marginTop:5}}>✓ Valid: 0x742d35Cc6634C0532925a3b844Bc454e4438f44e</div>
              </div>
            </div>
          )}
          {scanErr&&!addrErr&&<div style={{marginTop:10,background:"#0e0508",border:"1px solid #30100e",borderRadius:7,padding:"10px 14px",color:"#ff7a7a",fontSize:12}}>⚠ {scanErr}</div>}
          {scanning&&(
